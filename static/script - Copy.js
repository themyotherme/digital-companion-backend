    ...
    showQuizModal(questions, quizTitle) {
        ...
        const render = () => {
            if (quizDone) {
                ...
                html += `</ol>`;
                html += `<div style='text-align:right;'><button id='quiz-close-btn' style='margin-top:1em;'>Close</button></div>`;
                html += `</div>`;
            ...
            html += `<button id='quiz-prev-btn' ${current===0?'disabled':''}>Previous</button>`;
            html += `<button id='quiz-next-btn' ${current===questions.length-1?'disabled':''}>Next</button>`;
            html += `<button id='quiz-submit-btn' style='margin-left:auto;' ${(userAnswers.includes(null)||quizDone)?'disabled':''}>Submit</button>`;
            html += `<button id='quiz-close-btn' style='margin-left:1em;'>Close</button>`;
            html += `</div>`;
            html += `</div>`;
            modal.innerHTML = html;
            // Handlers
            document.getElementById('quiz-prev-btn').onclick = () => { current = Math.max(0, current-1); render(); };
            document.getElementById('quiz-next-btn').onclick = () => { current = Math.min(questions.length-1, current+1); render(); };
            document.getElementById('quiz-close-btn').onclick = () => {
                // Stop music if playing
                const audio = document.getElementById('quiz-music-audio');
                if (audio) { audio.pause(); audio.currentTime = 0; }
            ...
            document.getElementById('quiz-submit-btn').onclick = () => { if (!userAnswers.includes(null)) { quizDone = true; render(); } };
        };
        document.body.appendChild(modal);
        render();
    },
